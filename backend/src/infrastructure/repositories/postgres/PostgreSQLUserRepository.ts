...
export class PostgreSQLUserRepository implements UserRepository {
  ...
  async findByResetToken(token: string): Promise<User | null> {
    const { rows: [row] } = await query("SELECT * FROM users WHERE reset_token = $1", [token]);
    if (!row) return null;
    return this.mapRowToUser(row);
  }
  ...
  async create(user: User): Promise<void> {
    await query(`
      INSERT INTO users (id, username, email, password_hash, role, active, is_email_verified, email_verification_token, email_verification_sent_at, reset_token, reset_token_expires, created_at, updated_at)
  ...
  async update(user: User): Promise<void> {
    await query(`
      UPDATE users SET 
        ...
        is_email_verified = $6, email_verification_token = $7, email_verification_sent_at = $8, 
        reset_token = $9, reset_token_expires = $10, updated_at = NOW()
      WHERE id = $11
    `, [
